...
if(level=="landsdekkende"){
  ...
  da <- da[Kommune==location]
  di <- di[Kommune==location]
}
```
...
Figur 2 viser bruk av antibiotika i sykehjem per indikasjon. Andel forskrivninger av antibiotika (inkludert metenamin) til forebygging og behandling er spesifisert.  Metenamin (Hiprex) er i Nasjonal faglig retningslinje for antibiotikabruk i primærhelsetjenesten, kun anbefalt til forebygging av urinveisinfeksjoner.
...
Figure_ForskrivningerAvAntibiotikaPerIndikasjon(di=di,da=da,DATE_USE=DATE_USE,indikasjon="Nedre luftveisinfeksjon")
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, results='asis', fig.cap='\\label{fig:figs5}Forskrivning av antibiotika (virkestoff) til behandling av hud- og bløtdelsinfeksjoner'}
Figure_ForskrivningerAvAntibiotikaPerIndikasjon(di=di,da=da,DATE_USE=DATE_USE,indikasjon="Hud- og bløtdelsinfeksjon, ekskl. postoperativ sårinfeksjon")
```
