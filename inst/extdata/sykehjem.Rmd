...
# arg_pdf <- plan_pdf$analysis_get(2)$arg_pdf
####################################
...
} else if (arg_pdf$level == "fylke") {
  da <- da[Fylke == arg_pdf$location]
  di <- di[Fylke == arg_pdf$location]
} else if (arg_pdf$level == "kommune") {
  da <- da[Kommune == arg_pdf$location & Fylke == arg_pdf$fylke]
  di <- di[Kommune == arg_pdf$location & Fylke == arg_pdf$fylke]
}
...
plan$analysis_add(
  ...
  varGrouping = dplyr::case_when(
    arg_pdf$level == "landsdekkende" ~ "Fylke",
    arg_pdf$level == "fylke" ~ "Kommune",
    arg_pdf$level == "kommune" ~ "Institusjon"
  ),
  peopleLabel = "beboere"
)
