...
void Location::seir_step_day(int day, int locnum, float beta, float a, float gamma){
	int S_tmp = S;
	int E_tmp = E;
	int Ia_tmp = Ia;
	int I_tmp = I;
	...
	E_probs[num] = E;
	R_probs[num] = R;
	int ds; int de1; int de2; int dia; int di;
...
void Location::seir_step_night(int day, int locnum, float beta, float a, float gamma){
	int S_tmp = S;
	int E_tmp = E;
	int Ia_tmp = Ia;
	int I_tmp = I;
...
int main(int nargs, char ** argsv){
    ...
    for(int i_sim = 0; i_sim < N; ++i_sim){
		...
		/*	if(G_current.locations[40].S > 10){  // Number 40 is Oslo
			...
			for (int i = 0; i < n; ++i){
				G_current.locations[i].seir_step_night(i_day, i, beta, a, gamma);
				if(i_day == (M-1)){
    ...
    for(int i=0; i < n; ++i){
		for (int k = 0; k<2*M; ++k){
			out_data << i << " " << values[i][k][0]*1.0/N << " " << values[i][k][1]*1.0/N << " " << values[i][k][2]*1.0/N << " " << values[i][k][3]*1.0/N << " " << values[i][k][4]*1.0/N << endl;
